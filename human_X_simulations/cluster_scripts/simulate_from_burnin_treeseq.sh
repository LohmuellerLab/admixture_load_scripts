...
1:$( echo "( ${Nanc} * 10 + ${splittime} + ${Nanc} ) / ${scalingfactor}" | bc -l | xargs printf "%.0f") fitness(m1) {
    h = mut.mutationType.dominanceCoeff;
    if (isNULL(homozygous)) {
        return (1.0 + mut.selectionCoeff * 0.5)*(1.0 + mut.selectionCoeff * 0.5);
    } else if (homozygous) {
        return ((1.0 + 0.5*mut.selectionCoeff)*(1.0 + 0.5*mut.selectionCoeff));
    } else {
...
initialize() {
    ...
    info_lines = readFile("/u/home/b/bkim331/project-klohmueldata/bernard_data/admixture_simulations_x/sim_seq_info.txt");
...
$( echo "( ${Nanc} * 10 + ${splittime} + ${Nanc} ) / ${scalingfactor}" | bc -l | xargs printf "%.0f") late() {
    sim.treeSeqOutput("${cwd}/h${h}_full_" + simnum + "_p1.trees");
}
...
/u/home/b/bkim331/bin/slim -seed \${burnseed} slim_h${h}_full_split${s}.job
EOM
