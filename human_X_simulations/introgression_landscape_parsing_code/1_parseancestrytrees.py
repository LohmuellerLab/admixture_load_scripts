...
this script has been slightly modified to work with sex chromosomes
"""
...
def ancestry2(ts):
    ...
    #discard nodes that are null chromosomes
    p1 = [x for x in p1 
          if (pyslim.slim_metadata.decode_node(ts.node(x).metadata).is_null 
          == 0)]
    # get nodes alive today
    today = [x.id for x in ts.nodes() if x.time == 0.0]
    #discard nodes that are null chromosomes
    today = [x for x in today 
          if (pyslim.slim_metadata.decode_node(ts.node(x).metadata).is_null 
          == 0)]
